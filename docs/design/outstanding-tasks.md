...
### 1.2 Add temporary flip trigger
- Add a temporary visible button: `Flip Deck`
- Toggle `deck.isFlipped`
- Make every visible card rotate to its back/front together
- Confirm the whole deck feels like one object changing perspective

### 1.3 Test navigation while flipped
Manually check:
- active deck view shows backs
- focused mode shows backs
- swipe up/down still uses same card order
- focus/defocus still preserves continuity
- returning to the deck keeps flipped mode
- flipping does not reset active card

### 1.4 Decide final trigger
Options to evaluate after the temp button works:
- dedicated flip button
- small front/back toggle
- long press on deck/card
- two-finger gesture
- swipe/drag around card edge

For MVP, I’d probably use a clear explicit control rather than a hidden gesture.

### 1.5 Leave individual card flip mostly unchanged for now
Current MVP rule:
- individual card can still flip
- if user focuses or navigates while a card is individually flipped, it unflips
- later improve this with a visible unflip animation instead of a silent reset